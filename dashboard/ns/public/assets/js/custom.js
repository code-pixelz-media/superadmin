...
$(document).ready(function () {
  ...
  $(document).on("click", ".subscription-content-item .dzn-btn", function (event) {
      event.preventDefault(); // Prevent default link behavior

      const $button = $(this);
      const $headerPrice = $("#basic-price");

      // Find the price in the closest `.option-top` container
      const itemPriceText = $button.closest(".sci-left").find(".option-top h4").text().trim();
      const itemPrice = parseFloat(itemPriceText.replace(/[^0-9.]/g, "")); // Remove non-numeric characters except the decimal

      if (isNaN(itemPrice)) {
          console.error("Invalid item price:", itemPriceText);
          return; // Exit if the price is invalid
      }

      // Get the current total price from the header's data-price attribute
      const totalPriceText = $headerPrice.attr("data-price") || $headerPrice.text().trim(); // Fallback to text if data-price is empty
      const currentPrice = parseFloat(totalPriceText.replace(/[^0-9.]/g, "")); // Remove non-numeric characters except the decimal

      if (isNaN(currentPrice)) {
          console.error("Invalid total price:", totalPriceText);
          return; // Exit if the total price is invalid
      }

      // Check if the button already has the "price-added" class
      if ($button.hasClass("price-added")) {
          // Remove price and toggle the class
          const newPrice = currentPrice - itemPrice;
          $headerPrice.text(`$${newPrice}/m`).attr("data-price", newPrice);
          $button.removeClass("price-added").text("Add");
      } else {
          // Add price and toggle the class
          const newPrice = currentPrice + itemPrice;
          $headerPrice.text(`$${newPrice}/m`).attr("data-price", newPrice);
          $button.addClass("price-added").text("Remove");
      }
  });
});