...
if (isset($_GET['token'])) {
    ...
    $stmt = $dzyns_db->prepare("SELECT * FROM wooescrow_admin_users WHERE reset_token = :token AND reset_token_expires_at > NOW()");
    ...
    if ($user) {
        if (isset($_POST['submit'])) {
            ...
            if ($newPassword === $confirmPassword) {
                ...
                $stmt = $dzyns_db->prepare("UPDATE wooescrow_admin_users SET password = :password, reset_token = NULL, reset_token_expires_at = NULL WHERE reset_token = :token");
                ...
                header("refresh:3;url=/login/login.php"); // Redirect after 3 seconds
            } else {
