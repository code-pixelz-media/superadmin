...
class Dzyns_Database {
    ...
    private $db_name = 'license_verification';
    ...
    public function __construct()
    {
        ...
        if(HOST == 'localhost'){
            ...
            $this->host = 'localhost';
            $this->db_name = 'license_verification';
            $this->username = 'root';
            $this->password = 'root';
           ...
           $this->host = 'localhost';
           $this->db_name = 'wooescrow_superadmin';
           $this->username = 'wooescrow_superadmin';
           $this->password = 'root';
...
function storeOTP($email, $otp) {
    try {
        ...
        $stmt = $dzyns_db->prepare("UPDATE wooescrow_admin_users SET email_code = :otp, email_code_expires_at = :otp_expiration WHERE email = :email");
...
function updatePassword($email, $newPassword) {
    ...
    $stmt = $dzyns_db->prepare("UPDATE wooescrow_admin_users SET password = :password WHERE email = :email");
...
function generate_password_link($email) {
   // global $dzyns_db;
    ...
    $dzyns_database = new Dzyns_Database();
    $dzyns_db = $dzyns_database->dzyns_getConnection();
    $stmt = $dzyns_db->prepare("SELECT * FROM wooescrow_admin_users WHERE email = :email");
    $stmt->bindParam(':email', $email);
    $stmt->execute();
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($user) {
        ...
        $stmt = $dzyns_db->prepare("UPDATE wooescrow_admin_users SET reset_token = :token, reset_token_expires_at = :expiry WHERE email = :email");
        ...
        if ($stmt->execute()) {
            ...
            $resetLink = HOMEPAGE_URL."login/update-password.php?token=$token";
...
//// register page
  ...
  function registerUser($first_name, $last_name, $email, $password) {
    try {
        ...
        $stmt = $dzyns_db->prepare("SELECT id FROM " . PREFIX . "users WHERE email = :email");
        $stmt->execute(['email' => $email]);
        ...
        // Hash the password (consider stronger hashing methods like bcrypt)
        $hashed_password = hash('sha1', $password);

        // Insert new user into the database
        $stmt = $dzyns_db->prepare("
            INSERT INTO " . PREFIX . "users (first_name, last_name, email, password) 
        ...
        if ($result) {
            ...
            $body = "
                ...
                You can now log in using your registered email: <strong>$email</strong><br><br>
                <a href='" . HOMEPAGE_URL . "login'>Log in to your account</a><br><br>
                Best Regards,<br>
                The Team
            ";

            // Use PHPMailer to send the registration email
            try {
                $mail = new PHPMailer(true);
             ...
             //   $mail->isSMTP();
                ...
                // Set the sender and recipient
                $mail->setFrom('utsav@codepixelz.tech', 'DZYNS');
                ...
                // Send the email
                $mail->send();
                return true; // Registration successful and email sent
            } catch (Exception $e) {
                return "Registration successful, but the email could not be sent: " . $mail->ErrorInfo;
            }
        } else {
